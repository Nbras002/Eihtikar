...
يدعم مشروع احتكار التكامل مع قاعدة بيانات (PostgreSQL) باستخدام Drizzle ORM. **استخدام قاعدة البيانات اختياري:**

- إذا أردت تفعيل ميزات مثل حفظ بيانات اللاعبين أو الإحصائيات أو إدارة الجلسات المتقدمة، يجب إنشاء قاعدة بيانات PostgreSQL وإضافة متغير البيئة `DATABASE_URL` في إعدادات الخادم على Render.com.
...
1. أنشئ قاعدة بيانات PostgreSQL مُدارة على Render.com (أو أي مزود سحابي).
2. انسخ رابط الاتصال بقاعدة البيانات.
3. أضفه كمتغير بيئي باسم `DATABASE_URL` في إعدادات الخادم على Render.com.

**الخلاصة:**
- قاعدة البيانات **ليست مطلوبة** للتشغيل الأساسي، لكنها **مطلوبة** للميزات الكاملة.
